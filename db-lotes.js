...
function filaAlerta(r){return {id:texto(r.alert_id),loteId:texto(r.lot_id),usuario:texto(r.user_key),fechaAviso:texto(r.notify_date),enviada:Boolean(r.sent_at),activa:Boolean(r.active),vencimiento:texto(r.expiry_date),cantidad:Number(r.quantity)||0,articulo:texto(r.article),codigo:texto(r.code),rubro:texto(r.category)};}
async function listarAlertasProductoDb(codigo,usuario){await asegurarEsquemaLotes();const r=await query(`SELECT a.*,l.code,l.article,l.expiry_date,l.quantity,l.category FROM lot_alerts a JOIN lot_records l ON l.lot_id=a.lot_id WHERE l.code=$1 AND a.user_key=$2 AND a.active=TRUE ORDER BY l.expiry_date,a.notify_date`,[texto(codigo),texto(usuario).toLowerCase()]);return r.rows.map(filaAlerta);}
