...
async function calendars(){
  ...
  let sectores=[];
  try{
    const r=await fetch('/horarios/contexto',{cache:'no-store'});
  ...
  for(const sector of ids)for(const mes of months){
    try{
      const r=await fetch(`/horarios/calendario?sector=${encodeURIComponent(sector)}&mes=${mes}`,{cache:'no-store'});
      const d=await r.json();
