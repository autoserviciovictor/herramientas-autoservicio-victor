...
assert.match(notifications, /if \(!aceptar\) return;[\s\S]*await activarNotificaciones\(\)/, 'El permiso nativo solo debe solicitarse después de la acción del usuario');
assert.match(notifications, /window\.addEventListener\("autoservicio:sesion"[\s\S]*mostrarAvisoPermisoNotificaciones/, 'El chequeo debe ejecutarse al iniciar sesión');
assert.match(notifications, /if \(!event\.detail\?\.usuario\)[\s\S]*avisoPermisoUsuario = ""/, 'Cerrar sesión debe permitir evaluar correctamente al siguiente usuario');
...
assert.ok(sw.includes('NOTIFICACIONES_PERMISO_INGRESO_010926'), 'El service worker debe cambiar para forzar el refresco de notifications.js');
