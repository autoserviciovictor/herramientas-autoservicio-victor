const ZXING_SCRIPT_URL = "https://unpkg.com/@zxing/library@0.23.0/umd/index.min.js";
let promesaCargaZXing = null;

async function asegurarZXing() {
  if (globalThis.ZXing?.BrowserMultiFormatReader) return globalThis.ZXing;
  if (!promesaCargaZXing) {
    promesaCargaZXing = new Promise((resolve, reject) => {
      let script = document.querySelector('script[data-autoservicio-zxing="1"]');
      const completar = () => {
        if (globalThis.ZXing?.BrowserMultiFormatReader) resolve(globalThis.ZXing);
        else reject(new Error("El lector de códigos no quedó disponible."));
      };
      const fallar = () => reject(new Error("No se pudo descargar el lector de códigos."));
      if (!script) {
        script = document.createElement("script");
        script.src = ZXING_SCRIPT_URL;
        script.async = true;
        script.referrerPolicy = "no-referrer";
        script.dataset.autoservicioZxing = "1";
        document.head.appendChild(script);
      }
      script.addEventListener("load", completar, { once: true });
      script.addEventListener("error", fallar, { once: true });
      if (globalThis.ZXing?.BrowserMultiFormatReader) completar();
    }).catch((error) => {
      promesaCargaZXing = null;
      throw error;
    });
  }
  return promesaCargaZXing;
}
...
function detenerStreamsVideo() {
  ["video", "videoVencimientos", "videoReposicion", "videoPrecios", "videoEtiquetas"].forEach(
    (videoId) => {
      const video = document.getElementById(videoId);
...
function crearErrorCamara(error) {
  ...
  if (nombre === "NotAllowedError" || nombre === "PermissionDeniedError") {
    return new Error(
      "No se permitió usar la cámara. Habilitá el permiso de cámara en el navegador y volvé a intentarlo.",
    );
  }
  ...
  if (nombre === "NotReadableError" || nombre === "TrackStartError") {
    return new Error(
      "La cámara está siendo usada por otra aplicación. Cerrala y volvé a intentarlo.",
    );
  }
  if (
...
async function elegirCamaraTraseraPrincipal({
  ...
  const elegida = camaras
    .map((camara, index) => ({
      camara,
      puntos: puntuarCamara(
        camara,
        index,
        camaras.length,
        priorizarEnfoqueCercano,
      ),
    }))
...
async function mejorarEnfoque(videoId, sesion) {
  try {
    if (!camaraActiva || sesion !== sesionScanner || videoId !== videoActivoId)
      return;
    const track = obtenerTrackVideo(videoId);
    if (!track || !track.applyConstraints || track.readyState === "ended")
      return;
...
export async function iniciarScanner(videoId, callbackCodigo) {
  ...
  try {
    ...
    throw new Error(
      "No se pudo cargar el lector de códigos. Revisá la conexión e intentá nuevamente.",
    );
  }
  if (!document.getElementById(videoId))
    throw new Error("No se encontró el visor de la cámara.");
  if (typeof callbackCodigo !== "function")
    throw new Error("No se configuró la lectura del código.");
  ...
  try {
    ...
    camaraActiva = true;
    temporizadoresEnfoque.push(
      setTimeout(() => mejorarEnfoque(videoId, sesion), 350),
    );
    temporizadoresEnfoque.push(
      setTimeout(() => mejorarEnfoque(videoId, sesion), 1100),
    );
    ...
    if (esDispositivoIOS()) {
      temporizadoresEnfoque.push(
        setTimeout(() => mejorarEnfoque(videoId, sesion), 2200),
      );
      temporizadoresEnfoque.push(
        setTimeout(() => mejorarEnfoque(videoId, sesion), 4000),
      );
    }
  } catch (error) {
    detenerScanner();
...
if (!globalThis.__autoservicioScannerLifecycle) {
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden" && esDispositivoIOS())
      detenerScanner();
  });
}
